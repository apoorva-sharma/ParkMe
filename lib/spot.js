...
function Spot(spot_id, device_id, device_sensor_index) {
   ...
   var url = "https://"
   this.m_eventSource = new EventSource("https://api.spark.io/v1/" 
                                        + this.m_device_id + "/events/" 
                                        + this.m_eventName
                                        + "?access_token="+accessToken );

   this.m_eventSource.addEventListener(this.m_eventName, this.emitSensorChange);
}

Spot.prototype.__proto__ = EventEmitter.prototype;

Spot.prototype.emitSensorChange = function(e) {
   var data = {"spot_id": this.m_id, "state": e};
